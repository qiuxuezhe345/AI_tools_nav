...
import type { AdminTool, AdminToolCategory } from "@/lib/admin-tools";
...
export function ToolForm({ categories, initialTool, mode }: Props) {
  const router = useRouter();
  const [name, setName] = useState(initialTool?.name ?? "");
  const [slug, setSlug] = useState(initialTool?.slug ?? "");
  const [shortDescription, setShortDescription] = useState(
    initialTool?.short_description ?? "",
  );
  const [websiteUrl, setWebsiteUrl] = useState(initialTool?.website_url ?? "");
  const [categoryId, setCategoryId] = useState(
    initialTool?.category_id ? String(initialTool.category_id) : "",
  );
  const [logoUrl, setLogoUrl] = useState(initialTool?.logo_url ?? "");
  const [coverImageUrl, setCoverImageUrl] = useState(
    initialTool?.cover_image_url ?? "",
  );
  const [content, setContent] = useState(initialTool?.content ?? "");
  const [isHot, setIsHot] = useState(Boolean(initialTool?.is_hot));
  const [isNew, setIsNew] = useState(Boolean(initialTool?.is_new));
  const [status, setStatus] = useState<"published" | "hidden">(
    initialTool?.status === "hidden" ? "hidden" : "published",
  );
  ...
  function handleNameChange(value: string) {
    setName(value);
    if (!slug.trim() || slug === slugify(initialTool?.name ?? "")) {
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      try {
        const endpoint =
          mode === "create" ? "/api/admin/tools" : `/api/admin/tools/${initialTool?.id}`;
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="text-2xl font-semibold text-white">
            {mode === "create" ? "提交新工具" : "编辑 AI 工具"}
          </h2>
          <p className="mt-2 text-sm leading-6 text-white/60">
            {mode === "create"
              ? "提交后将直接写入 ai_tools 表。图片会上传到 Supabase Storage，Markdown 内容支持实时预览。"
              : "修改后将直接更新 ai_tools 表中的对应记录。"}
          </p>
        </div>
        <Button
          asChild
      ...
      <div className="space-y-8">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <label className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white">
            <Checkbox
              checked={isHot}
              onCheckedChange={(checked) => setIsHot(Boolean(checked))}
            />
            设为热门工具
          </label>
          <label className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white">
